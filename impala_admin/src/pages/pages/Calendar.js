  ...
  return (
    <Container >
      ...
      <Grid container maxWidth={"xl"}>
        ...
        <BookingView data={eventClickInfo.event.url} />
        </div> }<br/>
...
function CalendarItem({data}) {
  return (
    ...
    <Typography variant="overline"><GetUser data={data.event.id} /> / <GetCar data={data.event.title}/> / 
...
function BookingView({data}){
  console.log(data,"ji")
  let booking = useGetBookingById(data).docs
  return(
 booking ? <Card sx={{p:3}}>
 <Stack spacing={3}>
  <Typography variant="overline"><GetUser data={booking.app_user_id} /></Typography>
  <Typography variant="overline"><GetCar data={booking.car_id}/> </Typography>
  <Typography variant="overline">Start Date : { new Date(new firebase.firestore.Timestamp(booking?.startDate?.seconds, booking?.startDate?.nanoseconds).toDate()).toLocaleDateString()}</Typography>
  <Typography variant="overline">End Date : { new Date(new firebase.firestore.Timestamp(booking?.endDate?.seconds, booking?.endDate?.nanoseconds).toDate()).toLocaleDateString()}</Typography>
  <Typography variant="overline">K{fNumber(booking.totalAmount)}</Typography>
  <Typography variant="overline">{booking.status}</Typography>
  <Link  to={"/viewBooking/"+booking.id}
            state={{data:booking}}
             style={{ display: 'grid', color:"inherit", textDecoration:"none", justifyContent:"center",  }}>View</Link>
  </Stack>
 </Card>
 : ""
  )
